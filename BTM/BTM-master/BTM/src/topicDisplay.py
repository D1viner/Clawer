...
def perplexity(argvs):
    prob_sum = 0  # p(b)
    model_dir = argvs[8]  # 模型的存储路径
    K = int(argvs[1])  # 主题个数
    voca_pt = argvs[9]  # 词汇-id对应表路径
    test_corpus = argvs[7]  # 测试集路径
...
def dispTopics(pt, voca, pz):
    ...
    for l in open(pt):
        ...
        tmps = ' '.join(['%s:%f' % (voca[w], v) for w, v in wvs[:25]])  # 多少个词语
        topics.append((pz[k], tmps))
        k += 1

    print('topic_num\tp(z)\t\tTop words')
    i = 0
...
def WordCloud(argv):
    ...
    for l in open(zw_pt):
        ...
        wvs = zip(range(len(vs)), vs)
        wvs = sorted(wvs, key=lambda d: d[1], reverse=True)
        tmps = ' '.join(['%s' % voca[w] for w, v in wvs[:100]])
        # tmps = ' '.join(['%s:%f' % (voca[w], v) for w, v in wvs[:25]])  # 多少个词语
        topics.append(tmps)
        k += 1
    for i in range(5):
        txt = topics[i]
        w = wordcloud.WordCloud(width=4000,
                                height=3000,
                                font_path="msyh.ttc",
                                background_color='white').generate(txt)
        plt.imshow(w)
        plt.show()
        # filename = 'wordcloud_topic_' + str(i + 1) + '.png'
        # w.to_file(filename=filename)


def Cluster_graph(argv):
    model_dir = argv[1]
    K = int(argv[2])
    voca_pt = argv[3]
    voca = read_voca(voca_pt)
    ...
    for l in open(zw_pt):
        ...
        wvs = zip(range(len(vs)), vs)
        wvs = sorted(wvs, key=lambda d: d[1], reverse=True)
        tmps = ' '.join(['%s' % voca[w] for w, v in wvs])
        # tmps = ' '.join(['%s:%f' % (voca[w], v) for w, v in wvs[:25]])  # 多少个词语
        topics.append(tmps) #5个主题
    m=''
